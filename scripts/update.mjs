...
const NEARBY = (CFG.인근단지 || []);
const SIG = JSON.stringify({ AREA, REGIONS, 내집: CFG.내집, 목표: CFG.목표, 인근단지: NEARBY });
...
// 인근단지 — 단지이름으로 롤링 중위
function rollingNearby(store, months, aptName, kind, n = 3) {
  return months.map(ym => {
    const vals = [];
    let y = ym;
    for (let k = 0; k < n; k++) {
      const rows = store.월[y]?.인근단지?.[aptName]?.[kind] || [];
      rows.forEach(t => vals.push(t.가격));
      y = prevYm(y);
    }
    return r2(eok(median(vals)));
  });
...
const nearbyOut = NEARBY.map(c => {
  ...
  const recentNear = kind => {
    const out2 = [];
    for (const ym of [...shown].reverse()) {
      const rows = store.월[ym]?.인근단지?.[c.이름]?.[kind] || [];
      for (const t of rows) out2.push({ 날짜: `${ym.slice(2,4)}.${ym.slice(4)}.${pad2(t.일)}`, 가격: r2(eok(t.가격)), 면적: t.면적, 층: t.층 });
